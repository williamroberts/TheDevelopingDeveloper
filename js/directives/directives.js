...
theDevelopingDeveloperDirectives.directive('ddGmDirections', ['$modal',
	function($modal) {
		...
		function link(scope, element, attrs, ddGmMapCtrl) {
			...
			scope.showDirections = function() {
				...
				var modalOptions = {
					title: 'Directions',
					content: '1 + 2',//directionsResult,//'<div id="directions-container"></div>',
					actionButtonText: 'OK',
					closeButtonText: 'Close',
				};

				var modalController = function ($scope, $modalInstance, modalOptions) {
	...
	function () {
		...
		function controller($scope) {
			...
			return ({
				...
				calculateWaypointToRemove: calculateWaypointToRemove,
				canCalculateDirections: canCalculateDirections,
				calculateDirections: calculateDirections,
				canViewDirections: canViewDirections,
				getDirectionsResult: getDirectionsResult
			});
			...
			function addOrigin(newOrigin, oldOrigin) {
				removeOrigin(oldOrigin);
				origin = newOrigin;
				addMarker(newOrigin, 'https://maps.gstatic.com/mapfiles/markers2/boost-marker-mapview.png');
			}

			function removeOrigin(oldOrigin) {
				origin = '';
				removeMarker(oldOrigin);
			}

			function addWaypoint(newWaypoint) {
				addMarker(newWaypoint, 'https://maps.gstatic.com/mapfiles/markers2/icon_green.png');
			...
			function calculateWaypointToRemove(currentWaypoints, previousWaypoints) {
			...
			function addMarker(newMarker, iconFile) {
				...
				var infowindow = new google.maps.InfoWindow({
					content: '<p><strong>' + marker.name + '</strong><br />' + marker.address + '</p>'
				});
			...
			function canCalculateDirections() {
				return !!origin && waypoints.length > 0;
			}

			function calculateDirections(roundTrip) {
				var directionsDisplay = new google.maps.DirectionsRenderer({
					map: map,
				...
				var batches = placeWayPointsIntoBatches(copyOfWayPoints);
				...
				for (var i = 0; i < batches.length; i++) {
					...
					var request = buildDirectionsRequest(originOfBatch, wayPointsForReq, destinationOfBatch);
					...
					directionsService.route(request, function(response, status) {
						if (status == google.maps.DirectionsStatus.OK) {
							if (!directionsResult) { // First request
								...
								directionsResult.Pb.destination = response.Pb.destination;
								directionsResult.Pb.waypoints = directionsResult.Pb.waypoints.concat(response.Pb.waypoints);
			...
			function canViewDirections() {
				return true;
			}

			function getDirectionsResult() {
				return directionsResult;
			}

			function placeWayPointsIntoBatches(waypoints) {
			...
			function buildDirectionsRequest(origin, waypoints, destination) {
				return {
		...
		function link(scope, element, attrs, ctrl) {
			ctrl.initMap(angular.element(document.querySelector('#gmaps')));

			scope.canCalculateDirections = function() {
				return ctrl.canCalculateDirections();
			};

			scope.calculateDirections = function() {
				ctrl.calculateDirections(this.roundTrip);
			};

			scope.$watchCollection('markers', function(newItems, previousItems) {
				if (newItems.length > previousItems.length) {
					ctrl.addWaypoint(newItems[newItems.length - 1]);
				} else {
					ctrl.removeWaypoint(ctrl.calculateWaypointToRemove(newItems, previousItems));
				}
			});
			scope.$watch('origin', function(newOrigin, previousOrigin) {
				if (newOrigin) {
					ctrl.addOrigin(newOrigin, previousOrigin);	
				} else {
					ctrl.removeOrigin(previousOrigin);
				}
			});
